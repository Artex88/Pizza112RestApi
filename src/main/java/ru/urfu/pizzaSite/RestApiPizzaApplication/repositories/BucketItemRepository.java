...
public interface BucketItemRepository extends JpaRepository<BucketItem, Integer> {
    ...
    void updateBucketItemByQuantity(@Param("productVariant") ProductVariant productVariant, @Param("bucket") Bucket bucket);
    @Modifying
    @Query("UPDATE BucketItem SET quantity = quantity + 1 where product = :product and bucket = :bucket")
    void updateBucketItemByQuantity(@Param("product") Product product, @Param("bucket") Bucket bucket);
